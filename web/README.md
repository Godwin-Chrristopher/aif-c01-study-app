...
hard-question JSON batches to `web/hard-questions.js` or another
`hard-questions-*.js` file loaded by `web/index.html`; the app deduplicates that
session by question text when it renders.
...
4. Set the source to the main branch and the `/web` folder if that option is
   available.

If GitHub Pages does not show `/web` as a folder option, move these files to a
`docs/` folder or to the repository root, then publish from that location.
